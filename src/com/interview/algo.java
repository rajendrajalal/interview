...
for (int i = 0, j = str.length() - 1; i < j; ++i, --j) if (str.charAt(i) != str.charAt(j))//false binary palindrome
for (int i = inputringArray.length - 1; i > 0; i--) emptyArray += inputringArray[i] + " ";//reverse word order
for (int i = 0; i < str.length; i++) str[i] = new StringBuilder(str[i]).reverse().toString();//reverse each word
k = k % n;reverse(charArray, 0, n - k - 1); reverse(charArray, n - k, n - 1); reverse(charArray, 0, n - 1);//reversestring by offset k
if (!Character.isLetter(str.charAt(i)) || !(str.charAt(i) <= str.charAt(i + 1))) return false;//is string alphabetical
Arrays.sort(chr1Arr); Arrays.sort(chr2Arr);return new String(chr1Arr).equals(new String(chr2Arr));// is anagram
for(char c : str1.toCharArray()) if(str2.indexOf(c) != -1) res0++;// J="aA", S="aAAbbbb" output 3
ountCharacters(String str) return str.replaceAll("\\s", "").length();
...
/*depth*/1+Math.max(depth(root.left),depth(root.right));/*issymmetric*/ return root==null || isMirror(root.left,root.right);
