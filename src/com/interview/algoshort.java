...
Pair sum
for(i_n-2)for(j_i+1_n-1)01 02 03 12 13 23
for(i_n-1){key/val,val/key}
Max area
for(i_n-2)for(j_i+1_n-1)01 02 03 12 13 23 min(A[i],A[j])*(j-i)
i-0,j-n-1 while(i<j) A[i]<A[j])?i++:j--
Rainwater trap
for(i_n-1){while(left>0)while(right<n)}current_i_trap=min(max_A[left],max_A[right])-A[i]
while(i<j)A[i]>=A[j]?(update_maxright:update_totalwater),j--:
Two strings equal # counts as backspace- while(p1>=0 && p2>=0) or while(p1>=0 || p2>=0) check both while
long contiguos substring 
for(left_n-1)for(right=left_n-1){set}
for(right_n-1){key_char,val_index}
Palindrome-left right inwards or left right outwards or resevere string compare
str.replace([^A-Z,a-z,0-9],"").tolowercase
Palindrome remove atmost one char
inwards while(left<right) isPalind(left+1,right)||isPalin(left,right+1)
Reverse linked list- while(currentnode!=null) prev,next return prev
Reverse linked list pos m & n one pass- while(pos>=n && pos<=n)reverse
Flatten doubly list node can have child property doubly list-top down approach
while(currentnode!=null){while(childnode_next!=null)}
Linked list cycle- while(true)i,i+2 move
Valid parenthesis-{(:),[:],{:}}for(i_n-1)
Valid string remove fewest brackets- str.split("")-[]-for(i_n-1)[(_i,(_i]
Queue using two stacks- stack1-insert, stack2-remove
Quick sort-quickselect-kth smallest element include repeated elements
Binary search-return start & end index of target
...
/*reverse each word*/res[i_n] = new StringBuilder(str[i_n]).reverse().toString();or 1 string for(i_n)res=str.charAt(i)+res;
/*is string palindrome*/return (str == null || str.length() <= 1) || str.equals(new StringBuilder(str).reverse().toString());
/*false binary palindrome*/for (int i = 0, j = str.length() - 1; i < j; ++i, --j) if (str.charAt(i) != str.charAt(j))
/*reverse word order*/for (int i = str.length - 1; i > 0; i--) emptyArray += str[n_i] + " ";
/*reversestr by offset k*/k = k % n;reverse(charArray, 0/n-k/0, n - k - 1/n-1/n-1);
...
wordCountMap(List s)for(:s)if(!hm.containsKey(k)hm.put(k,1)else hm.put(k,hm.get(k)+1));
}
...
//alice-
    create each id bit array & xor all
    xor makes bit to 0-traverse bit array check bit 1 index as 1 in any id bit array after checking 1 digit
in any id bit array exclude or remove that id bit array.
//green city-
find each connected components & its no of nodes & total weight
    --sort each connected component by node by weight which is cities by trees.
    --then apply knaspack on given input.

        for i from 1 to noOfCities:
            if totalTrees=0 :
                return cities,trees
        cities/trees ratio choose ith max
        if item fits into knapsapk take all of it
            return the total value or cities and amounts or trees taken

    Given n = 5 and edges = [[0, 1], [1, 2], [2, 3], [3, 4]]
...
count--;
    ...
    public static void main(String[] args)
        ...
        int noOfTestCases = input.nextInt(); // Read number of sites.
        for(int i=0; i<noOfTestCases;i++){
            int totalCities= input.nextInt();
            int totalTrees= input.nextInt();
            int[] totalTreesArray = new int[totalTrees];
            for(int j=0; j<totalTrees;j++){
                totalTreesArray[j]= input.nextInt();
            }
            int noOfPaths= input.nextInt();
            WeightedQuickUnionUF uf = new WeightedQuickUnionUF(totalCities); // Initialize N components.
            for(int k=0; k<noOfPaths;k++)
            {
                int p = input.nextInt();
                int q = input.nextInt(); // Read pair to connect.
    ...
    public int backPackII(int m, int[] A-totalTreesArray, int[] V=connetedCities) {
        // m is the size or no of trees of knapsack,maximum value of using first i items under total size j
        int[] dp = new int[m + 1];
        for (int i = 0; i < A.length; i++) {
            for (int j = m; j >= A[i]; j--) {
...
public class ConnectedComponents {
	...
	private static int numberOfComponents(ArrayList<Integer>[] adj, int[] treesArray) {
		int result = 0;
		int n = adj.length;
		boolean[] visited = new boolean[n];
    ...
    public static void main(String[] args) {
        ...
        for(int test=0; test<noOfTestCases;test++){
            int totalCities= input.nextInt();
            ArrayList<Integer>[] adj = (ArrayList<Integer>[])new ArrayList[totalCities];
    ...
    private static List<Integer> allSubstring(String str){
        List<Integer> resList = new ArrayList<>();
        int res = 0;
        for(int i = 0; i< str.length(); i++){
          for(int j=i+1; j<=str.length(); j++){
            resList.add(Integer.parseInt(str.substring(i,j)));
            res+=Integer.parseInt(str.substring(i,j));
          }
        }
    ...
    private static void allSubsequences(String str){
      int index = -1;
      String curr = "";
      allSubsequencesHelper(str, str.length(), index, curr);
    }
    static void allSubsequencesHelper(String str, int n,
                                 int index, String curr)
      {
          ...
          for (int i = index + 1; i < n; i++) {
              curr += str.charAt(i);
              allSubsequencesHelper(str, n, i, curr);
              curr = curr.substring(0, curr.length() - 1);
          }
      }

    private static List<Integer> rotateArray(List<Integer> outputList, int rotateVal){
      int size= outputList.size();
      rotateVal= rotateVal% size;
      List<Integer> resList = new ArrayList<>();
      System.out.println(outputList);
      for(int i =0; i<outputList.size();i++){
        if(i<rotateVal) resList.add(outputList.get(size+i-rotateVal));
        else resList.add(outputList.get(i-rotateVal));
      }
      outputList=resList;
      return outputList;
    }
  ...
  // O(n) time, O(1) space
    ...
    private static void rotateHelper(int[] nums, int s, int t) {
        while (s < t) {
            int tmp = nums[s];
            nums[s] = nums[t];
            nums[t] = tmp;
  ...
  //  O(n) time, O(n) space
    private static int[] rotateArrayClockwise(int[] nums, int k) {
...
public int compareTo(Student st){
    if(name.len==st.name.len)
      return 0;
    else if(name.len>st.len)
      return 1;
    else
      return -1; //or return (this.name.len - st.name.len);
    }
}
